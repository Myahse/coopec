...
import {
  searchEtatSuiviClientele,
  type EtatSuiviClienteleRow,
} from '@/services/etat-suivi-clientele'
import {
...
type SortKey = Exclude<keyof EtatSuiviClienteleRow, 'rowKey'>
...
const COLUMNS: { key: SortKey; label: string; align?: 'left' | 'right'; filterable?: boolean }[] = [
  ...
  { key: 'nomClient', label: 'Nom du client', filterable: true },
  { key: 'contact', label: 'Contact', filterable: true },
  { key: 'montantPret', label: 'Montant Prêt', align: 'right', filterable: true },
  { key: 'numeroDomiciliation', label: 'Numéro de domiciliation', filterable: true },
  ...
  { key: 'impayes', label: 'Impayés', align: 'right', filterable: true },
]
...
export function SuiviClientelePage() {
  const f = useDashboardFilters()

  const [dateDebut, setDateDebut] = useState(defaultDateDebut)
  const [dateFin, setDateFin] = useState(todayIso)
  const [rows, setRows] = useState<EtatSuiviClienteleRow[]>([])
  ...
  const institutionLabel = useMemo(() => {
    if (f.institution !== 'Toutes') return f.institution.trim()
    return ''
  }, [f.institution])
  ...
  const loadList = useCallback(async () => {
    ...
    setError(null)
    try {
      const list = await searchEtatSuiviClientele(
        {
          dateDebut,
          dateFin,
          ...
          institution: agencyScope.institution ?? (institutionLabel || undefined),
        },
        agencyLabels,
      )
      setRows(list)
      ...
      setRows([])
      setError(err instanceof Error ? err.message : 'Impossible de charger le suivi clientèle')
    } finally {
      setIsLoading(false)
    }
  }, [agencyLabels, agencyScope, dateDebut, dateFin, institutionLabel])
  ...
  function exportXls() {
    exportJsonToXlsx(
      'etat_suivi_clientele',
      'Suivi clientèle',
      filteredSorted.map((r) => ({
        'Coopec créance': r.coopecCreance,
        'Agence Créance': r.agenceCreance,
        'Nom du client': r.nomClient,
        Contact: r.contact,
        'Montant Prêt': r.montantPret ?? '',
        'Numéro de domiciliation': r.numeroDomiciliation,
  ...
  return (
    <DashboardTablePageLayout
      title="État de suivi clientèle"
      cardTitle="Suivi clientèle"
      cardDescription="Synthèse prêt par client, agence de créance et recouvrement."
      alerts={
        error ? (
            ...
            {!isLoading
              ? tablePg.pageItems.map((r) => (
                  <tr key={r.rowKey} className={TABLE_ROW_CLASS}>
                    <td className={`${TABLE_TD_CLASS} max-w-[120px] truncate`}>{r.coopecCreance || '—'}</td>
                    <td className={`${TABLE_TD_CLASS} max-w-[140px] truncate`}>{r.agenceCreance || '—'}</td>
                    <td className={`${TABLE_TD_CLASS} max-w-[160px] truncate`}>{r.nomClient || '—'}</td>
                    <td className={TABLE_TD_CLASS}>{r.contact || '—'}</td>
                    <td className={`${TABLE_TD_CLASS} text-right tabular-nums`}>
                      {formatMontant(r.montantPret) || '—'}
                    </td>
                    <td className={TABLE_TD_MONO_CLASS}>{r.numeroDomiciliation || '—'}</td>
                    <td className={`${TABLE_TD_CLASS} text-right tabular-nums`}>
                      {formatMontant(r.cumulMontantCollecte) || '—'}
                    </td>
