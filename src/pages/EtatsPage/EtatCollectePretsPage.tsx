...
import {
  searchEtatCollectePrets,
  type EtatCollectePretAgregatRow,
...
const COLUMNS: { key: SortKey; label: string; align?: 'left' | 'right'; filterable?: boolean }[] = [
  ...
  { key: 'totalMontant', label: 'MT COLLECTÉ', align: 'right', filterable: true },
]
...
export function EtatCollectePretsPage() {
  const f = useDashboardFilters()

  const [dateDebut, setDateDebut] = useState(defaultDateDebut)
  const [dateFin, setDateFin] = useState(todayIso)
  const [rows, setRows] = useState<EtatCollectePretAgregatRow[]>([])
  ...
  const loadList = useCallback(async () => {
    ...
    setError(null)
    try {
      const list = await searchEtatCollectePrets(
        {
          dateDebut,
          dateFin,
      ...
      setRows([])
      setError(err instanceof Error ? err.message : 'Impossible de charger l’état collecte des prêts')
    } finally {
      setIsLoading(false)
    }
  ...
  return (
    <DashboardTablePageLayout
      title="État collecte des prêts"
      cardTitle="Collecte des prêts"
      cardDescription="Période et collectes prêt par agence et date de paiement."
      alerts={
        error ? (
        ...
        rows.length > 0 && !isLoading ? (
          <div className="flex flex-wrap items-end gap-4">
            ...
            <div className="flex min-w-[240px] flex-1 items-center gap-2">
              ...
              <Input readOnly className="h-8 text-xs tabular-nums" value={formatMontant(totalMontant)} />
            </div>
